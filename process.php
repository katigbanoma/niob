...
if (isset($_POST['surname']) && isset($_POST['other_names']) && isset($_POST['member_title']) && isset($_POST['gender'])
    ...
    $query = mysqli_query($connection, "SELECT * FROM `users` WHERE username='$email'");
    ...
    if (mysqli_num_rows($query) > 0) {
        echo "<script>alert('Account with this email address already exists!'); location.href='index.php';</script>";
        die();
    }
    ...
    $userQuery = "INSERT INTO `users` (username, password,created_at, updated_at) VALUES ('$email', '$hashed_password', CURRENT_DATE, CURRENT_DATE )";
